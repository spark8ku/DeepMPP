...
from D4CMPP.networks.src.ALIGNN import ALIGNNs
...
class network(nn.Module):
    def __init__(self, config):
        ...
        self.Linears = Linears(out_node_feats, hidden_dim, nn.LeakyReLU(), linear_layers, dropout, False, False, last=True) 

        self.solvent_layer = SolventLayer(config)

    def forward(self, graph, node_feats, edge_feats, triplet_feats, solv_graph, solv_node_feats, **kwargs):
        ...
        h = self.Linears(h)
        h = self.solvent_layer(h,solv_graph,solv_node_feats)
        return h
